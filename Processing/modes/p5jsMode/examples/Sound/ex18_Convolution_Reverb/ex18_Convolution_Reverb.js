...
function preload() {
  ...
  cVerb = createConvolver('data/bx-spring');

  // add Impulse Responses to cVerb.impulses array, in addition to bx-spring
  cVerb.addImpulse('data/small-plate');
  cVerb.addImpulse('data/drum');
  cVerb.addImpulse('data/beatbox');
  cVerb.addImpulse('data/concrete-tunnel');

  // load a sound that will be processed by the p5.ConvultionReverb
  sound = loadSound('data/Damscray_DancingTiger');
}
